...
async function loadSounds() {
    try {
        ...
        if (mainContent) {
            mainContent.innerHTML = `<div class="empty-state"><i class="fas fa-exclamation-triangle"></i><h3>Ошибка соединения</h3><p>Проверь настройки Appwrite или открой консоль (F12)</p></div>`;
        }
    }
}
...
async function handleUpload(e) {
    ...
    try {
        ...
        console.error("Ошибка загрузки:", err);
        if (statusDiv) statusDiv.innerHTML = "❌ Ошибка: " + err.message;
    }
}
